...
function generateCardFields() {
    ...
    // Limpar mensagem de resultado ao gerar campos
...
function findBestCombo() {
    ...
    const bestCards = bestCombo.slice(0, requiredCards).map(c => cards[c.card].name);

    const additionalCombos = [];
    for (let i = 1; i <= 10; i++) {
        const combo = bestCombo.slice(i * requiredCards, (i + 1) * requiredCards).map(c => cards[c.card].name);
        if (combo.length === requiredCards) {
            additionalCombos.push(combo.join(', '));
        }
    }

    const resultElement = document.getElementById('result');
    resultElement.textContent = `Melhor combinação de cartas: ${bestCards.join(', ')}`;
